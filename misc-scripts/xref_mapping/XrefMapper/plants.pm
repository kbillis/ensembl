...
sub transcript_names_from_gene {
  ...
  while ($get_genes->fetch()) {
    ...
    while ($get_transcripts->fetch) {
      ...
      #$core_dbi->prepare("INSERT IGNORE into xref (xref_id, external_db_id, dbprimary_acc, display_label, version, description, info_type, info_text) values(?, ?, ?, ?, 0, ?, 'MISC', ?)");
      $ins_xref_sth->execute($xref_id, $external_db_id, $label, $label, $description, $info_text);
      #$core_dbi->prepare("INSERT into object_xref (object_xref_id, ensembl_id, ensembl_object_type, xref_id) values(?, ?, 'Transcript', ?)");
      $ins_ox_sth->execute($ox_id, $transcript_id, $xref_id);
      #$core_dbi->prepare("UPDATE transcript t SET t.display_xref_id= ? WHERE t.transcript_id=?");
